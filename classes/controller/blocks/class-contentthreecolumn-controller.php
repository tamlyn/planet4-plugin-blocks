...
if ( ! class_exists( 'ContentThreeColumn_Controller' ) ) {
	...
	class ContentThreeColumn_Controller extends Controller {
		...
		public function prepare_template( $fields, $content, $shortcode_tag ) : string {
			...
			for ( $i = 1; $i < 4; $i++ ) {
				$img_array = wp_get_attachment_image_src( $fields[ "image_". __($i) ], 'medium_large' );
				if ( $img_array ) {
					$fields[ "alt_". __($i) ]   = get_post_meta( $fields[ "image_". __($i) ], '_wp_attachment_image_alt', true );
					$fields[ "image_". __($i) ] = $img_array[0];
				}
			}
