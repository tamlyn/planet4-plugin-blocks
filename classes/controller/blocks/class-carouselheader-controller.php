...
if ( ! class_exists( 'CarouselHeader_Controller' ) ) {
	...
	class CarouselHeader_Controller extends Controller {
			...
			for ( $i = 1; $i < 5; $i++ ) {
				$field   = [
					[
						...
						'label'       => sprintf( __( 'Select file for %s image', 'planet4-blocks' ), __($i) ),
						'attr'        => 'image_' . ${i},
						'type'        => 'attachment',
						'libraryType' => [ 'image' ],
					...
					[
						// translators: placeholder needs to represent the ordinal of the image, eg. 1st, 2nd etc.
						'label'       => sprintf( __( 'Select focus point for image %s', 'planet4-blocks' ), __($i) ) . '<img src="' . esc_url( plugins_url( '/planet4-plugin-blocks/admin/images/grid_9.png') ) . '" />',
						'attr'        => 'focus_image_' . ${i},
						'type'        => 'select',
						'options'     => [
					...
					[
						...
						'attr'  => 'subheader_' . __($i),
						'type'  => 'text',
						'meta'  => [
							// translators: placeholder needs to represent the ordinal of the image, eg. 1st, 2nd etc.
							'placeholder' => sprintf( __( 'Enter subheader of %s image', 'planet4-blocks' ), __($i) ),
							'data-plugin' => 'planet4-blocks',
						],
					],
					[
						...
						'meta'  => [
							// translators: placeholder needs to represent the ordinal of the image, eg. 1st, 2nd etc.
							'placeholder' => sprintf( __( 'Enter description of %s image', 'planet4-blocks' ), __($i) ),
							'data-plugin' => 'planet4-blocks',
						],
					],
					[
						...
						'meta'  => [
							// translators: placeholder needs to represent the ordinal of the image, eg. 1st, 2nd etc.
							'placeholder' => sprintf( __( 'Enter link text for %s image', 'planet4-blocks' ), __($i) ),
							'data-plugin' => 'planet4-blocks',
						],
					],
					[
						...
						'meta'  => [
							// translators: placeholder needs to represent the ordinal of the image, eg. 1st, 2nd etc.
							'placeholder' => sprintf( __( 'Enter link url for %s image', 'planet4-blocks' ), __($i) ),
							'data-plugin' => 'planet4-blocks',
						],
					],
				];
		...
		public function prepare_template( $attributes, $content, $shortcode_tag ) : string {
			...
			for ( $i = 1; $i < 5; $i++ ) {
				$temp_array      = [
					"header_".__($i)      => $attributes[ "header_".__($i) ] ?? '',
					"subheader_".__($i)   => $attributes[ "header_".__($i) ] ?? '',
					"description_".__($i) => $attributes[ "description_".__($i) ] ?? '',
					"image_".__($i)       => $attributes[ "image_".__($i) ] ?? '',
					"focus_image_".__($i) => $attributes[ "focus_image_".__($i) ] ?? '',
			...
			$total_images = 0;
			for ( $i = 1; $i < 5; $i++ ) {
				$image_id   = $attributes[ "image_".__($i) ];
				$temp_array = wp_get_attachment_image_src( $image_id, ['1118', '746'] );
				if ( false !== $temp_array && ! empty( $temp_array ) ) {
					$attributes[ "image_".__($i) ]          = $temp_array[0];
					$attributes[ "image_".__($i)."_srcset" ] = wp_calculate_image_srcset(['1118', '746'], wp_get_attachment_image_src( $image_id, 'full' )[0], wp_get_attachment_metadata( $image_id ));
					$total_images++;
				}
				$temp_image                     = wp_prepare_attachment_for_js( $image_id );
				$attributes[ "image_".__($i)."_alt" ] = $temp_image['alt'] ?? '';
			}
